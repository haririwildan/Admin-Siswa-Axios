// Tutup modal diluar konten (tes commit)
...
async function confirmSave() {
    ...
    await axios.post(`https://localhost:3000/students`, {
        name: nameForm,
        class: classForm,
        major: majorForm
    })
...
const updateData = async (id) => {
    if (checkUpdateInputs()) {
        ...
        document.getElementById('confirmButton').onclick = async () => {
            ...
            try {
                const response = await axios.patch(`https://localhost:3000/students/${id}`, {
                    name: nameForm,
                    class: classForm,
                    major: majorForm
                });
...
const confirmDelete = async () => {
    if (currentDeleteId) {
        await axios.delete(`https://localhost:3000/students/${currentDeleteId}`)
            .then((response) => {
                console.log(response)
                getData()
            })
...
const getData = async () => {
    await axios.get(`https://localhost:3000/students`)
        .then((response) => {
            let bucket = ``
